...
colorScroll.Plot = function(elementSelector, propertyIdentifier, rawColorPoints, scrollElementSelector) {
    var that = this;
    ...
    this.change = function() {
        ...
        if (current instanceof colorScroll.ColorPoint) {
            ...
            var portionOfElementScrolled = (scrollTop - current.start.position()) / ( current.end.position() - current.start.position() );
            newColor = current.start.mix(current.end, portionOfElementScrolled);
            that.active = true;
            action = function(){
        ...
        var css = {};
        css[propertyIdentifier] = newColor;
